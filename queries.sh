...
echo "$($PSQL "SELECT DISTINCT(name) FROM games LEFT JOIN teams ON games.opponent_id = teams.team_id OR teams.team_id = games.winner_id  WHERE year=2014 AND round='Eighth-Final'")"
